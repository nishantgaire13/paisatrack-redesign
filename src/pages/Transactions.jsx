...
function LedgerDoodle() {
  return (
    <svg width="80" height="90" viewBox="0 0 80 90" fill="none"
      ...
      <line x1="54" y1="18" x2="54" y2="82" stroke="#1c3a1c" strokeWidth="0.7"/>
    </svg>
  )
}
  ...
  return (
    <div style={{ padding: '36px 40px 60px', maxWidth: 1080, margin: '0 auto' }}>
      ...
      <div style={{
        ...
        {[
          { label: 'Showing',        value: `${filtered.length} entries`,        color: 'var(--text-primary)' },
          { label: 'Total Income',   value: formatNPR(totalFiltered.income),     color: 'var(--accent)'       },
          { label: 'Total Expenses', value: formatNPR(totalFiltered.expense),    color: 'var(--red)'          },
        ].map(({ label, value, color }) => (
          <div key={label} style={{ background: 'var(--bg-card)', padding: '16px 20px' }}>
            <p style={{ fontSize: 9, color: 'var(--text-faint)', letterSpacing: '0.12em', textTransform: 'uppercase', fontWeight: 500, marginBottom: 6 }}>{label}</p>
            <p style={{ fontFamily: 'var(--font-heading)', fontSize: 20, fontWeight: 600, color, letterSpacing: '-0.02em' }}>{value}</p>
          </div>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 10, marginBottom: 20, flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        <div style={{ display: 'flex', background: 'var(--bg-elevated)', borderRadius: 4, padding: 3, border: '1px solid var(--border-subtle)' }}>
          {['all','income','expense'].map(t => (
            <button key={t} onClick={() => setFilterType(t)} style={{
              ...
              color: filterType === t
                ? t === 'income' ? 'var(--accent)' : t === 'expense' ? 'var(--red)' : 'var(--text-primary)'
                : 'var(--text-muted)',
        ...
        {(search || filterType !== 'all' || filterCategory !== 'all') && (
          <button
            ...
            style={{ fontSize: 12, color: 'var(--text-muted)', padding: '7px 12px', borderRadius: 4, border: '1px solid var(--border-subtle)', background: 'var(--bg-card)', fontFamily: 'var(--font-body)', cursor: 'pointer' }}
          >
            Reset
          </button>
        )}
      ...
      <div style={{
        ...
        {filtered.length === 0 ? (
          ...
          <>
            ...
            <div style={{
              display: 'grid',
              gridTemplateColumns: '90px 1fr 130px 100px 120px 80px',
              padding: '10px 20px',
              ...
              return (
                ...
                >
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>
                    {formatDate(tx.date, profile.calendar)}
                  </span>
                  <div>
                    <p style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)' }}>{tx.merchant}</p>
                    {tx.description && (
                      <p style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 2 }}>{tx.description}</p>
                    )}
                  </div>
                    ...
                    {isDeleting ? (
                      <>
                        <button onClick={() => handleDelete(tx.id)}
                          style={{ color: 'var(--red)', padding: 4, borderRadius: 3, border: '1px solid var(--red)', fontSize: 10, fontFamily: 'var(--font-body)', background: 'transparent', cursor: 'pointer' }}>
                          <Check size={12} />
                        </button>
                        <button onClick={() => setDeletingId(null)}
                          style={{ color: 'var(--text-muted)', padding: 4, borderRadius: 3, border: '1px solid var(--border-default)', fontSize: 10, fontFamily: 'var(--font-body)', background: 'transparent', cursor: 'pointer' }}>
                          <X size={12} />
                        </button>
                      </>
                    ) : (
                      <>
                        <button onClick={() => setEditingTx(tx)}
                          style={{ color: 'var(--text-muted)', padding: 4, borderRadius: 3, transition: 'color 0.15s', cursor: 'pointer' }}>
                          <Pencil size={13} />
                        </button>
    ...
    <div style={{ position: 'fixed', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 9999 }}>
      ...
      <motion.div
        ...
        style={{
          ...
          maxHeight: '88vh', overflowY: 'auto', zIndex: 1,
        }}
      >
        ...
        <div style={{ display: 'flex', background: 'var(--bg-elevated)', borderRadius: 6, padding: 3, marginBottom: 20, border: '1px solid var(--border-subtle)' }}>
          {['expense','income'].map(t => (
            <button key={t} onClick={() => { setType(t); setCategory('') }} style={{
              flex: 1, padding: '8px 0', borderRadius: 4, fontWeight: 500, fontSize: 13,
